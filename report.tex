...
\begin{document}
   ...
   \section{Static space without gravity}
	...
	Previously, it was assumed that the intergalactic medium consists of neutral gas. But when astrophysicists {\bf James E. Gunn} and {\bf Bruce A. Peterson} calculated the expected level of 
        ...
        All the information for doing the Fourier transforms are stored in plans.
        Plans are of the type {\textsans fftw\_plan}. 
        
        These plans can be executed later by calling the function {\textsans fftw\_execute().}
        
        The multi-dimensional arrays passed to {\textsans fftw\_plan\_dft} are expected to be stored in row-major order. 
        ...
        $ i_{d-1} + n_{d-1}(i_{d-2}+ n_{d-2}(... + n_1i_0)). $
        ...
        \begin{itemize}
        	\item {\textsans fftw\_plan fftw\_plan\_dft\_1d(int n0, fftw\_complex *in, fftw\_complex *out, int sign, unsigned flags);}
        	\item {\textsans fftw\_plan fftw\_plan\_dft\_2d(int n0, int n1, fftw\_complex *in, fftw\_complex *out, int sign, unsigned flags);}
        	\item {\textsans fftw\_plan fftw\_plan\_dft\_3d(int n0, int n1, int n2, fftw\_complex *in, fftw\_complex *out, int sign, unsigned flags);}
        	\item {\textsans fftw\_plan fftw\_plan\_dft(int rank, const int *n, fftw\_complex *in, fftw\_complex *out, int sign, unsigned flags);}
        \end{itemize}
        
        A sample program to compute the Fourier transform of a Gaussian input signal is given below:
